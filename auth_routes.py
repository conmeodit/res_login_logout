...
async def register(request: RegisterRequest, response: Response):
    ...
    # Send OTP email
    email_sent = await send_otp_email(request.email, otp, "registration")
    if not email_sent:
        # Clean up temp registration if email fails
        await database.execute(
            sqlalchemy.delete(temp_registrations_table).where(
...
async def login(request: LoginRequest, response: Response):
    ...
    # Send OTP via email or SMS based on login method
    if is_email(request.identifier):
        otp_sent = await send_otp_email(user.email, otp, "login")
    else:
        otp_sent = await send_otp_sms(user.phone, otp)
    
    if not otp_sent:
        # Clean up temp session if OTP sending fails
        await database.execute(
            sqlalchemy.delete(temp_sessions_table).where(
